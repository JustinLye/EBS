...
public:
	...
	virtual void HandleDrawEvent(std::shared_ptr<Event>);
...
void ShapeModule::HandleContextEvent(std::shared_ptr<WindowEvent> event_ptr)
{
	if (event_ptr->GetName() == EventName::RENDER_CONTEXT_CHANGE_RELEASE)
	{
		...
		std::shared_ptr<Event> ack_event = std::make_shared<Event>(EventName::RENDER_CONTEXT_CHANGE_COMPLETE);
...
void ShapeModule::HandleDrawEvent(std::shared_ptr<Event> event_ptr)
{
	...
	std::shared_ptr<Event> ack_event = std::make_shared<Event>(EventName::RENDER_DRAWING_COMPLETE);
...
void ShapeModule::HandleAddShapeEvent(std::shared_ptr<ShapeEvent> event_ptr)
{
	...
	std::shared_ptr<Event> ack_event = std::make_shared<Event>(EventName::SHAPE_ADDSHAPE_COMPLETE);
	SendToSubscribers(ack_event);
}
